package com.example.airport.serial;
...
import com.example.airport.serial.thread.SerialPortReadThread;
...
public class SerialPortManager extends SerialPort {
    ...
    public boolean openSerialPort(File device, int baudRate) {
        ...
        Log.e("GG","openSerialPort: " + String.format("打开串口 %s  波特率 %s", device.getPath(), baudRate));

        // 校验串口权限
        if (!device.canRead() || !device.canWrite()) {
            boolean chmod777 = chmod777(device);
            if (!chmod777) {
                Log.e("GG","openSerialPort : " + baudRate + " 没有读写权限");
